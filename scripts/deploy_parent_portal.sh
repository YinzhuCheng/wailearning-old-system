...
PARENT_WEB_ROOT="${PARENT_WEB_ROOT:-/var/www/wailearning.xyz/parent}"
...
pushd "${SOURCE_DIR}/parent-portal" >/dev/null
npm ci
npm run build
popd >/dev/null
...
rsync -a --delete "${SOURCE_DIR}/parent-portal/dist/" "${PARENT_WEB_ROOT}/"
